-- Database setup in correct order
...
CREATE OR REPLACE FUNCTION public.handle_profile_role_assignment() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$ DECLARE user_count INTEGER; BEGIN SELECT COUNT(*) INTO user_count FROM profiles WHERE company_id = NEW.company_id; IF user_count = 1 THEN INSERT INTO user_roles (user_id, role) VALUES (NEW.user_id, 'admin') ON CONFLICT (user_id, role) DO NOTHING; ELSE INSERT INTO user_roles (user_id, role) VALUES (NEW.user_id, 'kayttaja') ON CONFLICT (user_id, role) DO NOTHING; END IF; RETURN NEW; END; $$;
CREATE OR REPLACE FUNCTION public.check_user_limit() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$ DECLARE license_record RECORD; current_user_count INTEGER; BEGIN SELECT * INTO license_record FROM public.licenses WHERE company_id = NEW.company_id; IF NOT FOUND THEN RAISE EXCEPTION 'Lisenssiä ei löydy tälle yritykselle'; END IF; SELECT COUNT(*) INTO current_user_count FROM public.profiles WHERE company_id = NEW.company_id; IF current_user_count >= license_record.max_users THEN RAISE EXCEPTION 'Käyttäjämäärä ylitetty. Lisenssisi sallii enintään % käyttäjää', license_record.max_users; END IF; RETURN NEW; END; $$;
...
CREATE OR REPLACE FUNCTION public.reduce_part_stock(part_id uuid, quantity integer) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$ BEGIN UPDATE public.varaosat SET saldo = GREATEST(0, saldo - quantity), updated_at = now() WHERE id = part_id AND is_active = true; END; $$;
...
DROP TRIGGER IF EXISTS after_profile_insert ON public.profiles; CREATE TRIGGER after_profile_insert AFTER INSERT ON public.profiles FOR EACH ROW EXECUTE FUNCTION public.handle_profile_role_assignment();
DROP TRIGGER IF EXISTS before_profile_insert_check_limit ON public.profiles; CREATE TRIGGER before_profile_insert_check_limit BEFORE INSERT ON public.profiles FOR EACH ROW EXECUTE FUNCTION public.check_user_limit();
DROP TRIGGER IF EXISTS set_asiakas_numero ON public.asiakkaat; CREATE TRIGGER set_asiakas_numero BEFORE INSERT ON public.asiakkaat FOR EACH ROW EXECUTE FUNCTION public.set_automatic_number();
DROP TRIGGER IF EXISTS set_huolto_numero ON public.Huollot; CREATE TRIGGER set_huolto_numero BEFORE INSERT ON public.Huollot FOR EACH ROW EXECUTE FUNCTION public.set_automatic_number();
DROP TRIGGER IF EXISTS set_lasku_numero ON public.laskut; CREATE TRIGGER set_lasku_numero BEFORE INSERT ON public.laskut FOR EACH ROW EXECUTE FUNCTION public.set_automatic_number();
...
CREATE TRIGGER update_hinnoittelu_updated_at BEFORE UPDATE ON public.hinnoittelu_asetukset FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_alv_updated_at BEFORE UPDATE ON public.alv_asetukset FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_lasku_asetukset_updated_at BEFORE UPDATE ON public.lasku_asetukset FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_maksutavat_updated_at BEFORE UPDATE ON public.maksutavat FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_takuu_updated_at BEFORE UPDATE ON public.takuu_asetukset FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_varasto_asetukset_updated_at BEFORE UPDATE ON public.varasto_asetukset FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_yrityksen_asetukset_updated_at BEFORE UPDATE ON public.yrityksen_asetukset FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_ilmoitus_asetukset_updated_at BEFORE UPDATE ON public.ilmoitus_asetukset FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_service_statuses_updated_at BEFORE UPDATE ON public.service_statuses FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_tekniikat_updated_at BEFORE UPDATE ON public.tekniikat FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_laite_valmistajat_updated_at BEFORE UPDATE ON public.laite_valmistajat FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE POLICY "Admins can insert roles" ON public.user_roles FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin') AND user_in_same_company(auth.uid(), user_id));
CREATE POLICY "Admins can update roles" ON public.user_roles FOR UPDATE USING (has_role(auth.uid(), 'admin') AND user_in_same_company(auth.uid(), user_id));
CREATE POLICY "Admins can delete roles" ON public.user_roles FOR DELETE USING (has_role(auth.uid(), 'admin') AND user_in_same_company(auth.uid(), user_id));
...
CREATE POLICY "Users can view company devices" ON public.Laitteet FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()));
CREATE POLICY "Admin and teknikko can insert devices" ON public.Laitteet FOR INSERT WITH CHECK (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teknikko')));
CREATE POLICY "Admin and teknikko can update devices" ON public.Laitteet FOR UPDATE USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teknikko')));
CREATE POLICY "Only admin can delete devices" ON public.Laitteet FOR DELETE USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Technicians can view own services" ON public.Huollot FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'teknikko') AND teknikko_id = auth.uid());
CREATE POLICY "Users can view all company services" ON public.Huollot FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'kayttaja'));
...
CREATE POLICY "Only admin can delete services" ON public.Huollot FOR DELETE USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Admin and teknikko can insert parts" ON public.varaosat FOR INSERT WITH CHECK (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teknikko')));
CREATE POLICY "Admin and teknikko can update parts" ON public.varaosat FOR UPDATE USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teknikko')));
CREATE POLICY "Only admin can delete parts" ON public.varaosat FOR DELETE USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Users can view company service parts" ON public.huolto_varaosat FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()));
CREATE POLICY "Admin and teknikko can insert service parts" ON public.huolto_varaosat FOR INSERT WITH CHECK (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teknikko')));
CREATE POLICY "Admin and teknikko can update service parts" ON public.huolto_varaosat FOR UPDATE USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teknikko')));
CREATE POLICY "Admin and teknikko can delete service parts" ON public.huolto_varaosat FOR DELETE USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teknikko')));
CREATE POLICY "Admin and teknikko can view invoices" ON public.laskut FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teknikko')));
CREATE POLICY "Admin and teknikko can insert invoices" ON public.laskut FOR INSERT WITH CHECK (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teknikko')));
CREATE POLICY "Admin and teknikko can update invoices" ON public.laskut FOR UPDATE USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teknikko')));
CREATE POLICY "Only admin can delete invoices" ON public.laskut FOR DELETE USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Only admin can manage numbering settings" ON public.numerointi_asetukset FOR ALL USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Admin can view pricing settings" ON public.hinnoittelu_asetukset FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Teknikko can view pricing settings" ON public.hinnoittelu_asetukset FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'teknikko'));
CREATE POLICY "Only admin can manage pricing settings" ON public.hinnoittelu_asetukset FOR ALL USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Users can view company VAT settings" ON public.alv_asetukset FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()));
CREATE POLICY "Only admin can manage VAT settings" ON public.alv_asetukset FOR ALL USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Users can view company invoice settings" ON public.lasku_asetukset FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()));
CREATE POLICY "Only admin can manage invoice settings" ON public.lasku_asetukset FOR ALL USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Users can view company payment methods" ON public.maksutavat FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()));
CREATE POLICY "Only admin can manage payment methods" ON public.maksutavat FOR ALL USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Users can view company warranty settings" ON public.takuu_asetukset FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()));
CREATE POLICY "Only admin can manage warranty settings" ON public.takuu_asetukset FOR ALL USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Users can view company inventory settings" ON public.varasto_asetukset FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()));
CREATE POLICY "Only admin can manage inventory settings" ON public.varasto_asetukset FOR ALL USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Users can view company settings" ON public.yrityksen_asetukset FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()));
CREATE POLICY "Only admin can manage company settings" ON public.yrityksen_asetukset FOR ALL USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Users can view company notification settings" ON public.ilmoitus_asetukset FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()));
CREATE POLICY "Only admin can manage notification settings" ON public.ilmoitus_asetukset FOR ALL USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Only admin can manage service statuses" ON public.service_statuses FOR ALL USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Users can view company technicians" ON public.tekniikat FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()));
CREATE POLICY "Teknikko can insert self" ON public.tekniikat FOR INSERT WITH CHECK (user_id = auth.uid() AND company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'teknikko'));
CREATE POLICY "Only admin can manage technicians" ON public.tekniikat FOR ALL USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Users can view company manufacturers" ON public.laite_valmistajat FOR SELECT USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()));
CREATE POLICY "Only admin can manage manufacturers" ON public.laite_valmistajat FOR ALL USING (company_id IN (SELECT company_id FROM profiles WHERE user_id = auth.uid()) AND has_role(auth.uid(), 'admin'));